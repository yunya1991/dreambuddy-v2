...
- Artifact Hub：默认 `127.0.0.1:3456`
...
- `ui-map` 中 `feed` 与 `chain` 节点的 implemented 入口都默认指向 `HUB_URL`
- 如需单独覆盖 `/feed` 基址，可使用 `OPS_UI_FEED_BASE_URL`
- 独立运行 `ops-ui` 时，建议显式设置 `HUB_URL=http://127.0.0.1:3456`
- `ops-ui` 不承载 `/feed` 或 `/chain` 内容本身，只负责进入 Hub 的真实页面
...
- 当前最小实现下，`ui-map` 中 `/feed` 已切到真实入口，但 `/feed/[category]` 与 `/feed/[category]/[artifactId]` 的架构图状态仍保留为后续分阶段收口。

### 6.2 与 `/chain` 的关系

- `/chain` 以遗留交易监控页为设计基线；
- 在目标关系中，`ops-ui` 将从 route、trace、task / result、审计层解释其执行过程。
- 当前最小实现下，`ui-map` 中 `/chain` 已切到 Hub 真实入口，用于进入链路监控首页与 `/chain/:workflowType` 工作流视图。
...
- `/chain` 双交易工作流的治理深度仍待继续扩展；
- 研究中台、市场化中台、董事会总览台之间的联动仍待实现。
- `ops-ui` 自身不承载 `/feed` 或 `/chain` 页面内容，仍通过 Hub 服务提供真实页面。
