...
## 实现状态（2026-05-22 / Task 6 验证）

- 已落地 `StrategyTaskOrder` / `StrategyExecutionRun` Prisma 模型与迁移，策略生命周期统一收敛到 `createStrategyWithLifecycle`、`applyStrategyWithTaskOrder`、`pauseStrategyWithTaskOrder`。
- 已闭环的主链为：创建/应用策略 -> 生成 `StrategyTaskOrder` + `StrategyTask` + `StrategyExecutionRun` -> 写入 `artifacts/trading/strategy_task_order_*.json` 产物 -> `GET /api/config/strategies` 返回 `taskOrders` / `executionRuns` -> Dashboard 以 `taskOrder` 为主链渲染已应用/运行中策略。
- `POST /api/config/strategies` 的 `apply` / `autoApply` 分支与 `POST /api/config/strategies/[id]/apply` 均已接入统一 `StrategyTaskOrder` 写入器；`POST /api/config/strategies/[id]/pause` 会同步暂停 task order 与 scheduler task。
- 已验证：`node --import tsx --test tests/strategy-task-order.test.ts tests/strategy-lifecycle-service.test.ts tests/strategy-artifacts.test.ts tests/strategy-view-model.test.ts tests/trading-panel-data.test.ts` 14/14 通过，`pnpm build` 通过。
- 当前剩余风险：全仓 `pnpm lint` 仍被历史问题阻塞，且 A4 推荐策略的真实上游推送链路本次仅验证了统一数据结构与产物格式，未在实时联调环境中完成端到端验收。

### Local Development Note

- 策略接口在开发期回退到 demo user 时，会自动确保该用户存在。
- 本地 reset 后可直接继续验证 `create` / `create+apply` / `feed` 主链，无需手动插入 demo user。
